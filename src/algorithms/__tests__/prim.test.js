...
describe('prim', () => {
  ...
  it('generates different mazes than recursiveBacktracking (same seed)', () => {
    const { grid: pGrid } = prim(10, 10, 42);
    const { grid: rbGrid } = recursiveBacktracking(10, 10, 42);
    // Two different algorithms should produce structurally different mazes
    let identical = true;
    for (let r = 0; r < 10 && identical; r++) {
      for (let c = 0; c < 10 && identical; c++) {
        if (JSON.stringify(pGrid[r][c].walls) !== JSON.stringify(rbGrid[r][c].walls)) {
          identical = false;
        }
      }
    }
