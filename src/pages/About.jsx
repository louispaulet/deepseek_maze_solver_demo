import { Grid3x3, Sparkles, Layers, Network, GitBranch, Binary, Search, ArrowDown, BarChart3, Star } from 'lucide-react';
import AlgoCardList from '../components/AlgoCardList';
...
const stack = ['Vite', 'React 19', 'TailwindCSS v4', 'React Router', 'Canvas API',
  'Vitest', 'ESLint v9', 'GitHub Pages', 'gh-pages'];
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-16">
      ...
      <section className="space-y-5 text-gray-300 leading-relaxed">
        ...
        <p>
          Maze generation and pathfinding are classic computer science problems. Building the
          full application — project scaffolding, algorithm logic, UI, animations, tests, and
          deployment — is a practical benchmark for reasoning, planning, and code generation.
        </p>

        <h2 className="text-xl font-semibold text-white mt-10 flex items-center gap-2">
          <Network size={20} className="text-indigo-400" /> Maze Generation</h2>
        <AlgoCardList items={GEN} icons={[Network, GitBranch, Binary]} />

        <h2 className="text-xl font-semibold text-white mt-10 flex items-center gap-2">
          <Search size={20} className="text-indigo-400" /> Pathfinding</h2>
        <AlgoCardList items={PATH} icons={[Search, ArrowDown, BarChart3, Star]} />

        <h2 className="text-xl font-semibold text-white mt-10 flex items-center gap-2">
          <Sparkles size={20} className="text-indigo-400" /> Features</h2>
        <ul className="list-disc list-inside space-y-2 ml-2 text-gray-400">
          {features.map((f) => <li key={f}>{f}</li>)}
        </ul>

        <h2 className="text-xl font-semibold text-white mt-10 flex items-center gap-2">
          <Layers size={20} className="text-indigo-400" /> Tech Stack</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-2">
          {stack.map((t) => (
            <span key={t} className="rounded-lg border border-gray-700 bg-gray-800/50 px-3 py-2 text-sm text-gray-300 text-center">
              {t}
            </span>
          ))}
        </div>
